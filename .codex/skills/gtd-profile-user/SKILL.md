...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$gtd-profile-user`.
- Treat all user text after `$gtd-profile-user` as `{{GTD_ARGS}}`.
...
<objective>
Generate a developer behavioral profile from session analysis (or questionnaire) and produce artifacts (USER-PROFILE.md, $gtd-dev-preferences, AGENTS.md section) that personalize the agent's responses.

Routes to the profile-user workflow which orchestrates the full flow: consent gate, session analysis or questionnaire fallback, profile generation, result display, and artifact selection.
</objective>

<execution_context>
@/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/workflows/profile-user.md
