...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$gtd-docs-update`.
- Treat all user text after `$gtd-docs-update` as `{{GTD_ARGS}}`.
...
- A flag is active only when its literal token appears in `{{GTD_ARGS}}`
- If a documented flag is absent from `{{GTD_ARGS}}`, treat it as inactive
...
- If `--force` and `--verify-only` both appear in `{{GTD_ARGS}}`, `--force` takes precedence
</objective>

<execution_context>
@/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/workflows/docs-update.md
</execution_context>
...
**Active flags must be derived from `{{GTD_ARGS}}`:**
- `--force` is active only if the literal `--force` token is present in `{{GTD_ARGS}}`
- `--verify-only` is active only if the literal `--verify-only` token is present in `{{GTD_ARGS}}`
