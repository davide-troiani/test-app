...
<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$gtd-debug`.
- Treat all user text after `$gtd-debug` as `{{GTD_ARGS}}`.
...
<execution_context>
@/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/workflows/debug.md
</execution_context>

<context>
User's input: {{GTD_ARGS}}

Parse subcommands and flags from {{GTD_ARGS}} BEFORE the active-session check:
- If {{GTD_ARGS}} starts with "list": SUBCMD=list, no further args
- If {{GTD_ARGS}} starts with "status ": SUBCMD=status, SLUG=remainder (trim whitespace)
- If {{GTD_ARGS}} starts with "continue ": SUBCMD=continue, SLUG=remainder (trim whitespace)
- If {{GTD_ARGS}} contains `--diagnose`: SUBCMD=debug, diagnose_only=true, strip `--diagnose` from description
