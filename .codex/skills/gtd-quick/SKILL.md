...
metadata:
  short-description: "Execute a quick task with GTD guarantees (atomic commits, state tracking) but skip optional agents"
---

<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$gtd-quick`.
- Treat all user text after `$gtd-quick` as `{{GTD_ARGS}}`.
...
<execution_context>
@/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/workflows/quick.md
</execution_context>
...
**Parse {{GTD_ARGS}} for subcommands FIRST:**

- If {{GTD_ARGS}} starts with "list": SUBCMD=list
- If {{GTD_ARGS}} starts with "status ": SUBCMD=status, SLUG=remainder (strip whitespace, sanitize)
- If {{GTD_ARGS}} starts with "resume ": SUBCMD=resume, SLUG=remainder (strip whitespace, sanitize)
- Otherwise: SUBCMD=run, pass full {{GTD_ARGS}} to the quick workflow as-is
...
<security_notes>
- Slugs from {{GTD_ARGS}} are sanitized before use in file paths: only [a-z0-9-] allowed, max 60 chars, reject ".." and "/"
