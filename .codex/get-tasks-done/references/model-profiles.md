...
Model profiles control which the agent model each GTD agent uses. This allows balancing quality vs token spend, or inheriting the currently selected session model.
...
When installed for a non-the agent runtime, the GTD installer sets `resolve_model_ids: "omit"` in `~/.gtd/defaults.json`. This returns an empty model parameter for all agents, so each agent uses the runtime's default model. No manual setup is needed.
