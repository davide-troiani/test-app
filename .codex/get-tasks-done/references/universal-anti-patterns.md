...
10. **NEVER** use non-GTD agent types (`general-purpose`, `Explore`, `Plan`, `Bash`, `feature-dev`, etc.) -- ALWAYS use `subagent_type: "gtd-{agent}"` (e.g., `gtd-phase-researcher`, `gtd-task-executor`, `gtd-planner`). GTD agents have project-aware prompts, audit logging, and workflow context. Generic agents bypass all of this.
...
15. **No direct Write/Edit to STATE.md or ROADMAP.md for mutations.** Always use `gtd-sdk query` for registered state/roadmap handlers (e.g. `state.update`, `state.advance-plan`, `roadmap.update-plan-progress`), or legacy `node …$gtd-tools.cjs` for CLI-only commands. Direct Write tool usage bypasses safe update logic and is unsafe in multi-session environments. Exception: first-time creation of STATE.md from template is allowed.
...
24. **Do not** check for `mode === 'auto'` or `mode === 'autonomous'` -- GTD uses `yolo` config flag. Check `yolo: true` for autonomous mode, absence or `false` for interactive mode.
25. **Prefer `gtd-sdk query`** for orchestration when a handler exists; when shelling out to the legacy CLI, use **`gtd-tools.cjs`** (not `gtd-tools.js` or any other filename) — GTD ships the programmatic API as CommonJS for Node.js CLI compatibility.
26. **Plan files MUST follow `{padded_phase}-{NN}-PLAN.md` pattern** (e.g., `01-01-PLAN.md`). Never use `PLAN-01.md`, `plan-01.md`, or any other variation -- gtd-tools detection depends on this exact pattern.
