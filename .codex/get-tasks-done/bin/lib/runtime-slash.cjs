...
/**
 * runtime-slash.cjs — single source of truth for emitting GTD slash-command
 ...
 *
 * Background: #2808 unified all GTD skill installs to register under the hyphen
 * form (`name: gtd-<cmd>`). The legacy colon form `/gtd:<cmd>` is no longer
 ...
 * `$gtd-<cmd>` form. This module is the only place the runtime should decide
 * which shape to emit.
 *
 *   - codex:                                   $gtd-<cmd>   (shell-var syntax)
 *   - claude, cursor, opencode, kilo, etc.:    /gtd-<cmd>
 *
 * The colon form is never emitted.
 */

function formatGtdSlash(commandName, runtime) {
  ...
  // both legacy `/gtd:<name>` and canonical hyphen-form input (plus the bare
  // `gtd:<name>` shorthand and codex `$gtd-<name>` shell-var input).
  ...
  // Defensive: a degenerate input like `/gtd:`, `gtd-`, or whitespace-only
  ...
  if (rt === 'codex') {
    // Codex skills are invoked as $gtd-<cmd> (shell-var syntax). The command
    ...
    return `$gtd-${token.toLowerCase()}${tail}`;
  }
 ...
 *
 *   process.env.GTD_RUNTIME  >  config.runtime  >  'claude'
 *
...
function resolveRuntime(projectDir) {
  if (process.env.GTD_RUNTIME) {
    return String(process.env.GTD_RUNTIME).toLowerCase();
  }
  if (projectDir) {
    try {
 ...
 * Equivalent to `formatGtdSlash(name, resolveRuntime(projectDir))`.
 */
function formatGtdSlashFor(projectDir, commandName) {
