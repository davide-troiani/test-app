 ...
 *
 * Manages the runtime enable/disable surface state (the `.gtd-surface.json` marker in
 * each runtime's skills dir) independently of the install-time profile marker
 * (`.gtd-profile`). Runtime config locations are resolved by callers.
 *
...
const SURFACE_FILE_NAME = '.gtd-surface.json';
...
function resolveSurface(runtimeConfigDir, manifest, clusterMap) {
  ...
  // Determine base profile name: from surface state or from .gtd-profile marker
 ...
 * Only touches files matching `gtd-` prefix or `*.md` in commandsDir.
 * Never touches non-`gtd-*` files.
 *
 * Steps:
 ...
 *  4. Sync: copy missing, delete superseded (gtd-only)
 *
 * @param {string} runtimeConfigDir
 * @param {string} commandsDir  runtime commands/gtd dir (resolved per-runtime by callers)
...
function applySurface(runtimeConfigDir, commandsDir, agentsDir, manifest, clusterMap) {
  ...
  _syncGtdDir(stagedSkills, commandsDir, 'commands');
  ...
  if (agentsDir && fs.existsSync(agentsDir)) {
    ...
    if (srcAgentsDir) {
      ...
      _syncGtdDir(stagedAgents, agentsDir, 'agents');
    }
  }
}
 ...
 * Removes gtd-prefixed .md files in dest not present in staged.
 * Never touches non-gtd files.
 *
 ...
 */
function _syncGtdDir(stagedDir, destDir, context) {
  ...
  // Remove gtd-only files from dest that aren't in staged set
  // For commands dir: all .md files are gtd skills
  // For agents dir: only gtd-* files
  const destEntries = fs.readdirSync(destDir).filter(f => f.endsWith('.md'));
  for (const file of destEntries) {
    if (context === 'agents' && !file.startsWith('gtd-')) continue;
...
/**
 * Find the install source commands/gtd directory.
 * Checks the runtime's `.gtd-source` marker (sibling of the surface state file),
 * then walks up from __dirname to find the installed package source.
 *
 * @param {string} runtimeConfigDir
 * @returns {string} path to install source commands/gtd
 */
function _findInstallSource(runtimeConfigDir) {
  // Check for .gtd-source marker
  const sourceMarker = path.join(runtimeConfigDir, '.gtd-source');
  ...
  // Walk up from this module's dir to find commands/gtd
  let dir = __dirname;
  for (let i = 0; i < 6; i++) {
    const candidate = path.join(dir, 'commands', 'gtd');
  ...
  return path.join(runtimeConfigDir, '..', 'commands', 'gtd');
}
...
function _findAgentsSource(runtimeConfigDir) {
  // Prefer .gtd-source sibling marker (commands/gtd) and derive agents from it.
  const sourceMarker = path.join(runtimeConfigDir, '.gtd-source');
  if (fs.existsSync(sourceMarker)) {
    try {
