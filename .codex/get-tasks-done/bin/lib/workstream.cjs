 ...
 *
 * When no workstreams/ directory exists, GTD operates in "flat mode" with
...
function cmdWorkstreamCreate(cwd, name, options, raw) {
  ...
  if (!fs.existsSync(baseDir)) {
    error(`.planning/ directory not found — run ${formatGtdSlash('new-project', resolveRuntime(cwd))} first`);
  }
