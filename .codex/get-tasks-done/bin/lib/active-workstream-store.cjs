 ...
 * CLI --ws > GTD_WORKSTREAM env > stored active workstream pointer.
 */
...
function resolveActiveWorkstream(cwd, args, env = process.env, deps = {}) {
  ...
  } else if (env && typeof env.GTD_WORKSTREAM === 'string' && env.GTD_WORKSTREAM.trim()) {
    ws = env.GTD_WORKSTREAM.trim();
    source = 'env';
  } else {
...
function applyResolvedWorkstreamEnv(resolution, env = process.env) {
  ...
  env.GTD_WORKSTREAM = resolution.ws;
}
