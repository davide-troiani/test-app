...
function cmdInitPlanPhase(cwd, phase, raw, options = {}) {
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'gtd-phase-researcher'),
    planner_model: resolveModelInternal(cwd, 'gtd-planner'),
    checker_model: resolveModelInternal(cwd, 'gtd-plan-checker'),
...
function cmdInitNewProject(cwd, raw) {
  ...
  const braveKeyFile = path.join(homedir, '.gtd', 'brave_api_key');
  const hasBraveSearch = !!(process.env.BRAVE_API_KEY || fs.existsSync(braveKeyFile));

  // Detect Firecrawl API key availability
  const firecrawlKeyFile = path.join(homedir, '.gtd', 'firecrawl_api_key');
  const hasFirecrawl = !!(process.env.FIRECRAWL_API_KEY || fs.existsSync(firecrawlKeyFile));

  // Detect Exa API key availability
  const exaKeyFile = path.join(homedir, '.gtd', 'exa_api_key');
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'gtd-project-researcher'),
    synthesizer_model: resolveModelInternal(cwd, 'gtd-research-synthesizer'),
    roadmapper_model: resolveModelInternal(cwd, 'gtd-roadmapper'),
...
function cmdInitQuick(cwd, description, raw) {
  ...
  const result = {
    // Models
    planner_model: resolveModelInternal(cwd, 'gtd-planner'),
    executor_model: resolveModelInternal(cwd, 'gtd-task-executor'),
    checker_model: resolveModelInternal(cwd, 'gtd-plan-checker'),
    verifier_model: resolveModelInternal(cwd, 'gtd-verifier'),
...
function cmdInitPhaseOp(cwd, phase, raw) {
  ...
  const result = {
    ...
    // string API keys (when user did `gtd-tools config-set brave_search XXX`).
...
function cmdInitMapCodebase(cwd, raw) {
  ...
  const result = {
    // Models
    mapper_model: resolveModelInternal(cwd, 'gtd-codebase-mapper'),
...
function cmdInitManager(cwd, raw) {
  ...
  // runtimes, $gtd-<cmd> shell-var for codex.
  ...
  if (!fs.existsSync(paths.roadmap)) {
    error(`No ROADMAP.md found. Run ${formatGtdSlash('new-milestone', _slashRuntime)} first.`);
  }
  if (!fs.existsSync(paths.state)) {
    error(`No STATE.md found. Run ${formatGtdSlash('new-milestone', _slashRuntime)} first.`);
  }
  ...
  // Compute recommended actions (task work > plan > discuss)
  ...
  for (const phase of phases) {
    ...
    if (phase.disk_status === 'planned' && phase.deps_satisfied) {
      recommendedActions.push({
        ...
        command: `${formatGtdSlash('export-phase-issues', _slashRuntime)} ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `${formatGtdSlash('plan-phase', _slashRuntime)} ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `${formatGtdSlash('discuss-phase', _slashRuntime)} ${phase.number}`,
      });
    }
  }
  ...
  const sanitizeFlags = (raw) => {
    ...
    if (!safe) {
      process.stderr.write(`gtd-tools: warning: manager.flags contains invalid tokens, ignoring: ${val}\n`);
      return '';
    }
    return val;
  };
...
function cmdInitNewWorkspace(cwd, raw) {
  ...
  const defaultBase = path.join(homedir, 'gtd-workspaces');
 ...
 * @param {string} agentType - The agent type (e.g., 'gtd-task-executor', 'gtd-planner')
 ...
 * Used by workflows: SKILLS=$(node "$TOOLS" agent-skills gtd-task-executor 2>/dev/null)
 */
 ...
 *   installation: { gtd_skills_installed: boolean, legacy_claude_commands_installed: boolean },
...
function buildSkillManifest(cwd, skillsDir = null) {
  ...
  const canonicalRoots = skillsDir ? [{
    ...
    },
    {
      ...
      path: path.join(os.homedir(), '.claude', 'get-tasks-done', 'skills'),
      scope: 'import-only',
      kind: 'skills',
      deprecated: true,
    },
    {
      ...
      path: path.join(os.homedir(), '.claude', 'commands', 'gtd'),
