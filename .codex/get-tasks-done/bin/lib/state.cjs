...
// hasn't changed within the same gtd-tools invocation.
...
function stateReplaceFieldWithFallback(content, primary, fallback, value) {
  ...
  process.stderr.write(
    `[gtd-tools] WARNING: STATE.md field "${primary}"${fallback ? ` (fallback: "${fallback}")` : ''} not found — update skipped. ` +
...
function buildStateFrontmatter(bodyContent, cwd) {
  ...
  const fm = { gtd_state_version: '1.0' };
...
function cmdStateBeginPhase(cwd, phaseNumber, phaseName, planCount, raw) {
  ...
  readModifyWriteStateMd(statePath, (content) => {
    ...
    // Update Last Activity (safe to update on resume — tracks when task orchestration flow ran)
...
/**
 * Write a WAITING.json signal file when GTD hits a decision point.
 * External watchers (fswatch, polling, orchestrators) can detect this.
 * File is written to .planning/WAITING.json (or .gtd/WAITING.json if .gtd exists).
 * Fixes #1034.
 */
function cmdSignalWaiting(cwd, type, question, options, phase, raw) {
  const gtdDir = fs.existsSync(path.join(cwd, '.gtd')) ? path.join(cwd, '.gtd') : planningDir(cwd);
  const waitingPath = path.join(gtdDir, 'WAITING.json');
...
function cmdSignalResume(cwd, raw) {
  const paths = [
    path.join(cwd, '.gtd', 'WAITING.json'),
...
function cmdStateMilestoneSwitch(cwd, version, name, raw) {
  ...
  try {
    ...
    const fm = {
      gtd_state_version: existingFm.gtd_state_version || '1.0',
      milestone: version,
 ...
 * Implements the `gtd state complete-phase` subcommand (issue #2735).
 */
