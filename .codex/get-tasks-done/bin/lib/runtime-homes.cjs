 ...
 * the correct global skills directory for every GTD-supported runtime.
 *
 ...
 *   hermes  — GTD skills nest under skills/gtd/<skillName>/ (not the flat
...
function getGlobalConfigDir(runtime) {
  ...
  switch (runtime) {
    ...
    // ── Hermes Agent ─────────────────────────────────────────────────────────
    // Note: skills use a nested layout (skills/gtd/<skill>/) — see getGlobalSkillDir().
    case 'hermes':
 ...
 * Hermes: <configDir>/skills/gtd  (nested category layout — #2841)
...
function getGlobalSkillsBase(runtime) {
  ...
  if (runtime === 'hermes') return path.join(configDir, 'skills', 'gtd');
 ...
 *
 * @param {string} runtime
 * @param {string} skillName - e.g. 'gtd-task-executor'
 * @returns {string|null}
 */
