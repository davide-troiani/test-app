...
Parse `{{GTD_ARGS}}` for the idea summary.

First, check for an enrich flag:

```bash
if echo "{{GTD_ARGS}}" | grep -qE '\-\-enrich[[:space:]]+SEED-[0-9]+'; then
  ENRICH_TARGET=$(echo "{{GTD_ARGS}}" | grep -oE 'SEED-[0-9]+')
...
If `{{GTD_ARGS}}` is non-empty and contains no `--enrich` flag, treat the full value as `$IDEA` (no prompt).

Only prompt for the idea when `{{GTD_ARGS}}` is empty and no enrich target is present. Store the response as `$IDEA`.
</step>
