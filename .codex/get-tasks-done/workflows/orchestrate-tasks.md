...
<read_only_mode>
If `{{GTD_ARGS}}` contains `--dry-run`, resolve the requested scope to exact child
...
1. Resolve `{{GTD_ARGS}}` before calling the SDK utility.
...
<execution_flow>
1. Run the Start Gate. If `{{GTD_ARGS}}` contains `--dry-run`, report the Start
