...
<template>
@/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/templates/UAT.md
</template>

<process>

<step name="initialize" priority="first">
If {{GTD_ARGS}} contains a phase number, load context:

```bash
GTD_WS=""
echo "{{GTD_ARGS}}" | grep -qE -- '--ws[[:space:]]+[^[:space:]]+' && GTD_WS=$(echo "{{GTD_ARGS}}" | grep -oE -- '--ws[[:space:]]+[^[:space:]]+')
PHASE_ARG=$(echo "{{GTD_ARGS}}" | sed -E 's/--ws[[:space:]]+[^[:space:]]+//g' | xargs)
...
**If active sessions exist AND no {{GTD_ARGS}} provided:**
...
**If active sessions exist AND {{GTD_ARGS}} provided:**
...
**If no active sessions AND {{GTD_ARGS}} provided:**
...
**MVP-mode UAT framing.** When `MVP_MODE=true`, follow the rules in `@/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/references/verify-mvp-mode.md`. Briefly:
...
Read and follow `/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/workflows/transition.md`.
...
- Follow @/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/workflows/diagnose-issues.md
...
`$gtd-work-task-issue --phase {phase} --gaps-only`
