...
@/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/references/ui-brand.md
@/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/references/revision-loop.md
@/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/references/gate-prompts.md
...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/references/tdd.md` so gate enforcement rules are available during planning.
...
Extract from {{GTD_ARGS}}: phase number (integer or decimal like `2.1`), flags (`--research`, `--skip-research`, `--research-phase <N>`, `--gaps`, `--skip-verify`, `--skip-ui`, `--prd <filepath>`, `--ingest <path-or-glob>`, `--ingest-format <auto|nygard|madr|narrative>`, `--reviews`, `--text`, `--bounce`, `--skip-bounce`, `--chunked`, `--mvp`).
...
RESEARCH_ONLY=false
VIEW_ONLY=false
if [[ "{{GTD_ARGS}}" =~ --research-phase[[:space:]]+([0-9]+(\.[0-9]+)?) ]]; then
  RESEARCH_ONLY=true
  PHASE="${BASH_REMATCH[1]}"
fi
if $RESEARCH_ONLY && [[ "{{GTD_ARGS}}" =~ (^|[[:space:]])--view([[:space:]]|$) ]]; then
  VIEW_ONLY=true
fi
```

Set `TEXT_MODE=true` if `--text` is present in {{GTD_ARGS}} OR `text_mode` from init JSON is `true`. When `TEXT_MODE` is active, replace every `AskUserQuestion` call with a plain-text numbered list and ask the user to type their choice number. This is required for Claude Code remote sessions (`/rc` mode) where TUI menus don't work through the the agent App.
...
```bash
MVP_FLAG_ARG=""
if [[ "{{GTD_ARGS}}" =~ (^|[[:space:]])--mvp([[:space:]]|$) ]]; then MVP_FLAG_ARG="--cli-flag"; fi
```
...
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `@/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/references/skeleton-template.md`.
...
Extract express-path args from {{GTD_ARGS}}: `PRD_FILE` (`--prd <filepath>`), `INGEST_PATH` (`--ingest <path-or-glob>`), and optional `INGEST_FORMAT` (`--ingest-format <auto|nygard|madr|narrative>`, default `auto`).
...
CHUNKED_MODE=false
if [[ "{{GTD_ARGS}}" =~ --chunked ]] || [[ "$CHUNKED_CFG" == "true" ]]; then
  CHUNKED_MODE=true
fi
```
...
**If found:**
1. Read template: `/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/templates/VALIDATION.md`
...
**If UI-SPEC.md missing AND `--skip-ui` flag is present in {{GTD_ARGS}}:** Skip silently to step 6.
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/references/tdd.md to all eligible tasks:
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `@/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
...
${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Follow vertical-slice planning rules from @/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/references/planner-mvp-mode.md. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
</planning_context>
...
if [ "$POST_PLANNING_GAPS" = "true" ]; then
  node "/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/bin/gtd-tools.cjs" gap-analysis --phase-dir "${PHASE_DIR}"
fi
```
...
1. Parse `--auto` and `--chain` flags from {{GTD_ARGS}}
...
```bash
if ([[ "{{GTD_ARGS}}" =~ --auto ]] || [[ "{{GTD_ARGS}}" =~ --chain ]]) && [[ "$AUTO_CHAIN" != "true" ]]; then
