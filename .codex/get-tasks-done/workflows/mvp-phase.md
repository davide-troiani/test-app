...
<required_reading>
@/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/references/user-story-template.md
@/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/references/spidr-splitting.md
@/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/references/planner-mvp-mode.md
</required_reading>

<runtime_note>
**Copilot (VS Code):** Use `vscode_askquestions` wherever this workflow calls `AskUserQuestion`. They are equivalent.

**TEXT_MODE fallback:** Set TEXT_MODE=true if `--text` is present in `{{GTD_ARGS}}` OR `text_mode` from init JSON is true. When TEXT_MODE is active, replace every AskUserQuestion call with a plain-text numbered list and ask the user to type their choice number.
</runtime_note>
...
Extract the phase number from `{{GTD_ARGS}}` (integer or decimal like `2.1`). Optional flag: `--force` (allow operating on `in_progress` / `completed` phases).
...
Normalize per `@/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/references/phase-argument-parsing.md` (zero-pad integer phases to two digits).
...
**Status guard.** If the phase is `in_progress` (has plans but not complete) or `completed`, refuse unless `--force` is in `{{GTD_ARGS}}`:
...
Run three sequential `AskUserQuestion` calls. Each is free-text. After all three, assemble into the canonical sentence per `@/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/references/user-story-template.md`:
...
Run the SPIDR rules from `@/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/references/spidr-splitting.md`. Briefly:
