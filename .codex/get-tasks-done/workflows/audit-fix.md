...
- gtd-task-executor — executes a specific, scoped code change
</available_agent_types>
...
```bash
INIT=$(gtd-sdk query audit-uat 2>/dev/null || echo "{}")
...
**b. Run tests:**
```bash
AUDIT_TEST_CMD=$(gtd-sdk query config-get workflow.test_command --default "" 2>/dev/null || true)
