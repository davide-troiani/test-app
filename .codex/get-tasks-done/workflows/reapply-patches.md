...
if [ -z "$PATCHES_DIR" ]; then
  if [ -d "$HOME/.config/kilo/gtd-local-patches" ]; then
    ...
    PATCHES_DIR="/Users/davide/repos/get-tasks-done-demo-app/.codex/gtd-local-patches"
  fi
fi
...
b. Identify ALL differences, then classify each as:
   - **Mechanical drift** — path substitutions (e.g. `/Users/xxx/.claude/` → `/Users/davide/repos/get-tasks-done-demo-app/.codex/`), variable additions (`${GTD_WS}`, `${AGENT_SKILLS_*}`), error handling additions (`|| true`)
