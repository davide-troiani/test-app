...
Extract from {{GTD_ARGS}}: phase number, reviewer flags (`--codex`, `--gemini`, `--claude`, `--opencode`, `--ollama`, `--lm-studio`, `--llama-cpp`, `--all`), `--max-cycles N`, `--text`, `--ws`.

```bash
PHASE=$(echo "{{GTD_ARGS}}" | grep -oE '[0-9]+\.?[0-9]*' | head -1)

REVIEWER_FLAGS=""
echo "{{GTD_ARGS}}" | grep -q '\-\-codex' && REVIEWER_FLAGS="$REVIEWER_FLAGS --codex"
echo "{{GTD_ARGS}}" | grep -q '\-\-gemini' && REVIEWER_FLAGS="$REVIEWER_FLAGS --gemini"
echo "{{GTD_ARGS}}" | grep -q '\-\-claude' && REVIEWER_FLAGS="$REVIEWER_FLAGS --claude"
echo "{{GTD_ARGS}}" | grep -q '\-\-opencode' && REVIEWER_FLAGS="$REVIEWER_FLAGS --opencode"
echo "{{GTD_ARGS}}" | grep -q '\-\-ollama' && REVIEWER_FLAGS="$REVIEWER_FLAGS --ollama"
echo "{{GTD_ARGS}}" | grep -q '\-\-lm-studio' && REVIEWER_FLAGS="$REVIEWER_FLAGS --lm-studio"
echo "{{GTD_ARGS}}" | grep -q '\-\-llama-cpp' && REVIEWER_FLAGS="$REVIEWER_FLAGS --llama-cpp"
echo "{{GTD_ARGS}}" | grep -q '\-\-all' && REVIEWER_FLAGS="$REVIEWER_FLAGS --all"
if [ -z "$REVIEWER_FLAGS" ]; then REVIEWER_FLAGS="--codex"; fi

MAX_CYCLES=$(echo "{{GTD_ARGS}}" | grep -oE '\-\-max-cycles\s+[0-9]+' | awk '{print $2}')
if [ -z "$MAX_CYCLES" ]; then MAX_CYCLES=3; fi

GTD_WS=""
echo "{{GTD_ARGS}}" | grep -qE '\-\-ws\s+\S+' && GTD_WS=$(echo "{{GTD_ARGS}}" | grep -oE '\-\-ws\s+\S+')
```
...
```bash
INIT=$(node "/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/bin/gtd-tools.cjs" init plan-phase "$PHASE")
...
Set `TEXT_MODE=true` if `--text` is present in {{GTD_ARGS}} OR `text_mode` from init JSON is `true`. When `TEXT_MODE` is active, replace every `AskUserQuestion` call with a plain-text numbered list and ask the user to type their choice number.

## 3. Validate Phase + Pre-flight Gate

```bash
PHASE_INFO=$(node "/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/bin/gtd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
```bash
node "/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/bin/gtd-tools.cjs" state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```
