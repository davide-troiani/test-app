...
1. If `--force` is present in `{{GTD_ARGS}}`: skip this check, include CONTRIBUTING.md in the queue.
...
1. If `--force` is present in `{{GTD_ARGS}}`: treat all docs as mode: regenerate, skip to detect_runtime_capabilities.
2. If `--verify-only` is present in `{{GTD_ARGS}}`: skip to verify_only_report (do not continue to detect_runtime_capabilities).
...
**Skip condition:** If `--verify-only` is present in `{{GTD_ARGS}}`, this step was already handled by `verify_only_report` (early exit). Skip.
...
**Reached when `--verify-only` is present in `{{GTD_ARGS}}`.** This is an early-exit step — do not proceed to dispatch, generation, commit, or report steps after this step.
