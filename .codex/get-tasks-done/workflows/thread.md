...
**Parse {{GTD_ARGS}} to determine mode:**
...
If {{GTD_ARGS}} matches an existing thread name:
...
If {{GTD_ARGS}} is a new description (no matching thread file):

1. Generate slug from description:
   ```bash
   SLUG=$(gtd-sdk query generate-slug "{{GTD_ARGS}}" --raw)
   ```
