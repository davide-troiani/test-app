...
Parse `{{GTD_ARGS}}` for `--from N`, `--to N`, `--only N`, and `--interactive` flags:

```bash
FROM_PHASE=""
if echo "{{GTD_ARGS}}" | grep -qE '\-\-from\s+[0-9]'; then
  FROM_PHASE=$(echo "{{GTD_ARGS}}" | grep -oE '\-\-from\s+[0-9]+\.?[0-9]*' | awk '{print $2}')
fi

TO_PHASE=""
if echo "{{GTD_ARGS}}" | grep -qE '\-\-to\s+[0-9]'; then
  TO_PHASE=$(echo "{{GTD_ARGS}}" | grep -oE '\-\-to\s+[0-9]+\.?[0-9]*' | awk '{print $2}')
fi

ONLY_PHASE=""
if echo "{{GTD_ARGS}}" | grep -qE '\-\-only\s+[0-9]'; then
  ONLY_PHASE=$(echo "{{GTD_ARGS}}" | grep -oE '\-\-only\s+[0-9]+\.?[0-9]*' | awk '{print $2}')
  FROM_PHASE="$ONLY_PHASE"
fi

INTERACTIVE=""
if echo "{{GTD_ARGS}}" | grep -q '\-\-interactive'; then
  INTERACTIVE="true"
fi
```
...
Read and execute: `/Users/davide/repos/get-tasks-done-demo-app/.codex/get-tasks-done/references/autonomous-smart-discuss.md`
