...
/**
 * Deterministic latest-version check for /gtd-update (#2992).
 *
 * The /gtd-update workflow's check_latest_version step was previously
 * prescribed in LLM-driven prose ("run `npm view @ai-is-gonna/get-tasks-done
 * version`"). The executing model could shortcut the prescription and
 * invent npm queries against wrong-shaped names (`@get-tasks-done/cli`,
 * `get-tasks-done-cli`, `gtd`), all of which 404 or — worse — return an
...
const PACKAGE_NAME = '@ai-is-gonna/get-tasks-done';
...
function checkLatestVersion(opts = {}) {
  ...
  // Bounded at 15s so a hung registry doesn't block /gtd-update (#2993 CR).
  const defaultSpawn = () => {
