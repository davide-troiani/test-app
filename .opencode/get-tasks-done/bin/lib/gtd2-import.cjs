...
/**
 * gtd2-import — Reverse migration from GTD-2 (.gtd/) to GTD v1 (.planning/)
 *
 * Reads a GTD-2 project directory structure and produces a complete
 * .planning/ artifact tree in GTD v1 format.
 *
 * GTD-2 hierarchy:  Milestone → Slice → Task
 * GTD v1 hierarchy: Milestone (in ROADMAP.md) → Phase → Plan
 *
 * Mapping rules:
...
/**
 * Find the .gtd/ directory starting from a project root.
 * Returns the absolute path or null if not found.
 */
function findGtd2Root(startPath) {
  if (path.basename(startPath) === '.gtd' && fs.existsSync(startPath)) {
    return startPath;
  }
  const candidate = path.join(startPath, '.gtd');
...
/**
 * Parse the ## Slices section from a GTD-2 milestone ROADMAP.md.
...
/**
 * Parse the milestone title from the first heading in a GTD-2 ROADMAP.md.
 * Format: # M001: Title
 */
...
/**
 * Parse a task title from a GTD-2 T##-PLAN.md.
 * Format: # T01: Title
 */
...
/**
 * Parse the ## Description body from a GTD-2 task plan.
 */
...
/**
 * Parse ## Must-Haves items from a GTD-2 task plan.
 */
...
/**
 * Read all task plan files from a GTD-2 tasks/ directory.
 */
...
/**
 * Parse a complete GTD-2 .gtd/ directory into a structured representation.
 */
function parseGtd2(gtdDir) {
  const data = {
    projectContent: readOptional(path.join(gtdDir, 'PROJECT.md')),
    requirements: readOptional(path.join(gtdDir, 'REQUIREMENTS.md')),
    milestones: [],
  };

  const milestonesBase = path.join(gtdDir, 'milestones');
...
/**
 * Build a GTD v1 PLAN.md from a GTD-2 task.
 */
...
/**
 * Build a GTD v1 SUMMARY.md from a GTD-2 task summary.
 * Strips the GTD-2 frontmatter and preserves the body.
 */
function buildSummaryMd(task, phasePrefix, planPrefix) {
  const raw = task.summary || '';
  // Strip GTD-2 frontmatter block (--- ... ---) if present
  ...
  return [
    ...
    '---',
    '',
    body || 'Task completed (migrated from GTD-2).',
    '',
  ].join('\n');
}

/**
 * Build a GTD v1 XX-CONTEXT.md from a GTD-2 slice.
 */
function buildContextMd(slice, phasePrefix) {
  const lines = [
    `# Phase ${phasePrefix} Context`,
    '',
    `Migrated from GTD-2 slice ${slice.id}: ${slice.title}`,
  ];
...
/**
 * Build the GTD v1 ROADMAP.md with milestone-sectioned format.
 */
...
/**
 * Build the GTD v1 STATE.md reflecting the current position in the project.
 */
function buildStateMd(phaseMap) {
  ...
  return [
    ...
    `Status: ${status}`,
    `Last activity: ${today} — Migrated from GTD-2`,
    '',
    `Progress: ${bar} ${pct}%`,
    '',
    '## Accumulated Context',
    ...
    '### Decisions',
    '',
    'Migrated from GTD-2. Review PROJECT.md for key decisions.',
    '',
    '### Blockers/Concerns',
    '',
    'None.',
    ...
    '',
    `Last session: ${today}`,
    'Stopped at: Migration from GTD-2 completed',
    'Resume file: None',
    '',
  ].join('\n');
}
...
/**
 * Convert parsed GTD-2 data into a map of relative path → file content.
 * All paths are relative to the .planning/ root.
 */
function buildPlanningArtifacts(gtd2Data) {
  const artifacts = new Map();

  // Passthrough files
  artifacts.set('PROJECT.md', gtd2Data.projectContent || '# Project\n\n(Migrated from GTD-2)\n');
  if (gtd2Data.requirements) {
    artifacts.set('REQUIREMENTS.md', gtd2Data.requirements);
  }
  ...
  const phaseMap = [];
  let phaseNum = 1;
  for (const milestone of gtd2Data.milestones) {
  ...
  artifacts.set('ROADMAP.md', buildRoadmapMd(gtd2Data.milestones, phaseMap));
 ...
 */
function buildPreview(gtd2Data, artifacts, projectDir) {
  ...
  const totalSlices = gtd2Data.milestones.reduce((s, m) => s + m.slices.length, 0);
  const doneSlices = gtd2Data.milestones.reduce((s, m) => s + m.slices.filter(sl => sl.done).length, 0);
  const allTasks = gtd2Data.milestones.flatMap(m => m.slices.flatMap(sl => sl.tasks));
  const doneTasks = allTasks.filter(t => t.done).length;

  lines.push('');
  lines.push(`Milestones: ${gtd2Data.milestones.length}`);
  ...
  lines.push('  - GTD-2 cost/token ledger (no v1 equivalent)');
  const { formatGtdSlash, resolveRuntime } = require('./runtime-slash.cjs');
  lines.push(`  - GTD-2 database state (rebuilt from files on first ${formatGtdSlash('health', resolveRuntime(projectDir))})`);
...
/**
 * Entry point called from gtd-tools.cjs.
 * Supports: --force, --dry-run, --path <dir>
 */
function cmdFromGtd2(args, cwd, raw) {
  ...
  const gtdDir = findGtd2Root(projectDir);
  if (!gtdDir) {
    return output({ success: false, error: `No .gtd/ directory found in ${projectDir}` }, raw);
  }

  const planningRoot = path.join(path.dirname(gtdDir), '.planning');
  ...
  const gtd2Data = parseGtd2(gtdDir);
  const artifacts = buildPlanningArtifacts(gtd2Data);
  ...
  const preview = buildPreview(gtd2Data, artifacts, projectDir);
  ...
  return output({
    ...
    milestones: gtd2Data.milestones.length,
    preview,
  }, raw);
}
