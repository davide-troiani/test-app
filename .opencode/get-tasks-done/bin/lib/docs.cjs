 ...
 * with GTD marker detection, doc tooling detection, monorepo awareness, and
...
/**
 * Check whether a file begins with the GTD doc writer marker.
 * Reads the first 500 bytes only — avoids loading large files.
 *
 * @param {string} filePath - Absolute path to the file
 * @returns {boolean}
 */
function hasGtdMarker(filePath) {
  try {
    const buf = Buffer.alloc(500);
    const fd = fs.openSync(filePath, 'r');
    const bytesRead = fs.readSync(fd, buf, 0, 500, 0);
    fs.closeSync(fd);
    return buf.slice(0, bytesRead).toString('utf-8').includes(GTD_MARKER);
  } catch {
    return false;
  }
}
 ...
 * @returns {Array<{path: string, has_gtd_marker: boolean}>}
 */
 ...
 *
 * @example
 * node gtd-tools.cjs docs-init --raw
 *
...
function cmdDocsInit(cwd, raw) {
  ...
  const result = {
    doc_writer_model: resolveModelInternal(cwd, 'gtd-doc-writer'),
