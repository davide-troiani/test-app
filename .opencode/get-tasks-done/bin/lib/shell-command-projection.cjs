 ...
 * that GTD writes into runtime config or prints for copy/paste. This module
...
function buildWindowsShimTriple(shimSrc) {
  ...
  return {
    invocation,
    eol: { cmd: '\r\n', ps1: '\n', sh: '\n' },
    fileNames: { cmd: 'gtd-sdk.cmd', ps1: 'gtd-sdk.ps1', sh: 'gtd-sdk' },
...
function formatSdkPathDiagnostic({ shimDir, platform, runDir }) {
  ...
  if (shimDir) {
    ...
    actionLines.push('npm install -g @ai-is-gonna/get-tasks-done');
  }
  const npxNoteLines = isNpx
    ? [
        "Note: you're running via npx. For a persistent shim,",
        'install globally instead: npm install -g @ai-is-gonna/get-tasks-done',
      ]
    : [];
