 ...
 *
 * Background: every installed `gtd-*` skill costs eager system-prompt tokens
 * because runtimes (Claude Code, opencode, etc.) enumerate skill descriptions
 * in `<available_skills>` on every turn. With 66 skills + 33 agents GTD alone
 ...
 *  - core     — task-based skills covering the main project loop
 *  - standard — core + phase management and workspace skills
 *  - full     — all skills (previous default, '*' sentinel)
 * Profiles compose: --profile=core,issue-tasks resolves to
 * union(closure(core), closure(issue-tasks)).
 ...
 *  - loadSkillsManifest: parse requires: frontmatter from commands/gtd/*.md
 ...
 *  - readActiveProfile / writeActiveProfile: .gtd-profile marker persistence
 *  - resolveEffectiveProfile: explicit flag > .gtd-profile marker > full
 *  - mostRestrictiveProfile: resolve multi-runtime disagreement to smallest set
 *
 * Companion module (Phase 2): get-tasks-done/bin/lib/surface.cjs owns the
 * runtime /gtd:surface command. It reuses stageSkillsForProfile and
 * stageAgentsForProfile from this module for cluster-level enable/disable
 * without reinstall, persisting state in <runtimeConfigDir>/.gtd-surface.json.
 *
 ...
 *
 * Composition: --profile=core,issue-tasks resolves to
 * union(closure(core), closure(issue-tasks)).
 */
 ...
 * since GTD enforces flow-style arrays for requires:.
 *
 ...
 * Scans the full content for `gtd-<stem>` patterns that correspond to
 * real agent files. Returns all unique `gtd-*` stems found in the body.
 *
 * The caller is responsible for filtering by which agents actually exist —
 * this function returns all syntactically valid `gtd-*` matches.
 *
 * @param {string} content full file content
 * @returns {string[]} deduplicated agent stems like ['gtd-planner', 'gtd-task-executor']
 */
function parseCallsAgents(content) {
  // Match word-boundary gtd-<stem> patterns; stems are lowercase letters and hyphens.
  // We use a regex that matches `gtd-` followed by one or more lowercase-alpha-or-hyphen chars.
  // This catches `gtd-planner`, `gtd-plan-checker`, etc. in prose and code.
  const matches = content.match(/\bgtd-[a-z][a-z-]*/g);
  if (!matches) return [];
  // Deduplicate
...
/**
 * Load the requires: dependency graph from a commands/gtd directory.
 * Also derives calls_agents for each skill by scanning the body text for
 * `gtd-*` agent name references. Agent stems are stored under the special
 * key `_calls_agents_<stem>` so they don't conflict with skill stems.
 *
 * @param {string} commandsDir absolute path to commands/gtd/
...
/**
 * Stage a filtered copy of commands/gtd for a resolved profile.
 * In full mode (skills === '*') returns srcDir unchanged (no-op).
 *
 * @param {string} srcDir absolute path to commands/gtd
...
function stageSkillsForProfile(srcDir, resolvedProfile) {
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gtd-profile-skills-'));
  try {
 ...
 * For tiered profiles, copies only agents whose full stem (e.g. 'gtd-planner')
...
function stageAgentsForProfile(srcAgentsDir, resolvedProfile) {
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gtd-profile-agents-'));
  try {
    if (resolvedProfile.agents instanceof Set && resolvedProfile.agents.size > 0) {
      ...
      for (const entry of entries) {
        if (!entry.isFile()) continue;
        if (!entry.name.endsWith('.md')) continue;
        // Agent stem is the full filename without extension, e.g. "gtd-planner"
        const stem = entry.name.slice(0, -3);
 ...
 */
const PROFILE_RANK = Object.freeze(['core', 'issue-tasks', 'standard', 'full']);
 ...
 *
 * Ordering (most to least restrictive): core < issue-tasks < standard < full.
 * Composed profiles (e.g. 'core,issue-tasks') and unknown profiles are treated as
...
function resolveEffectiveProfile({ requestedProfileName, targetDir }) {
  ...
  // 2. Marker-driven (gtd update path)
...
/**
 * Stage a filtered copy of the source commands/gtd directory.
...
function stageSkillsForMode(srcDir, mode) {
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gtd-minimal-skills-'));
  try {
    const entries = fs.readdirSync(srcDir, { withFileTypes: true });
    for (const entry of entries) {
      if (!entry.isFile()) continue;
