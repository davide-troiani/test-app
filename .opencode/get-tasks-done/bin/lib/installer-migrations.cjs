...
const INSTALL_STATE_NAME = 'gtd-install-state.json';
const INSTALL_MIGRATION_LOCK_NAME = 'gtd-install-migration.lock';
...
function applyInstallerMigrationPlan({ configDir, plan, now = () => new Date().toISOString() }) {
  ...
  const journalRelPath = path.posix.join('gtd-migration-journal', `${runId}.json`);
  const journalPath = path.join(configDir, journalRelPath);
  const rollbackRootRelPath = path.posix.join('gtd-migration-journal', `${runId}-rollback`);
  const rollbackRoot = path.join(configDir, rollbackRootRelPath);
  const backupRootRelPath = path.posix.join('gtd-migration-journal', `${runId}-backups`);
