...
/**
 * Remove stale gtd-* temp files/dirs older than maxAgeMs (default: 5 minutes).
 * Runs opportunistically before each new temp file write to prevent unbounded accumulation.
 * @param {string} prefix - filename prefix to match (e.g., 'gtd-')
 * @param {object} opts
 ...
 */
/**
 * Dedicated GTD temp directory: path.join(os.tmpdir(), 'gtd').
 * Created on first use. Keeps GTD temp files isolated from the system
 * temp directory so reap scans only GTD files (#1975).
 */
const GTD_TEMP_DIR = path.join(require('os').tmpdir(), 'gtd');

function ensureGtdTempDir() {
  platformEnsureDir(GTD_TEMP_DIR);
}

function reapStaleTempFiles(prefix = 'gtd-', { maxAgeMs = 5 * 60 * 1000, dirsOnly = false } = {}) {
  try {
    ensureGtdTempDir();
    const now = Date.now();
    const entries = fs.readdirSync(GTD_TEMP_DIR);
    for (const entry of entries) {
      if (!entry.startsWith(prefix)) continue;
      const fullPath = path.join(GTD_TEMP_DIR, entry);
      try {
...
function output(result, raw, rawValue) {
  ...
  if (raw && rawValue !== undefined) {
    ...
    if (json.length > 50000) {
      reapStaleTempFiles();
      ensureGtdTempDir();
      const tmpPath = path.join(GTD_TEMP_DIR, `gtd-${Date.now()}.json`);
 ...
 * instead of plain "Error: <message>" text. Set by gtd-tools.cjs when the
...
function loadConfig(cwd, options = {}) {
  ...
  // When GTD_WORKSTREAM is set, load root config first so workstream config
  ...
  try {
    ...
    if (unknownKeys.length > 0) {
      ...
      if (!_warnedUnknownConfigKeys.has(warnKey)) {
        ...
        process.stderr.write(
          `gtd-tools: warning: unknown config key(s) in .planning/config.json: ${unknownKeys.join(', ')} — these will be ignored\n`
        );
      }
    }
    ...
    // Fall back to ~/.gtd/defaults.json only for truly pre-project contexts (#1683)
    // If .planning/ exists, the project is initialized — just missing config.json.
    // When GTD_WORKSTREAM is set and root config was loaded, the workstream config
    ...
    try {
      const home = process.env.GTD_HOME || os.homedir();
      const globalDefaultsPath = path.join(home, '.gtd', 'defaults.json');
...
function pruneOrphanedWorktrees(repoRoot) {
  try {
    ...
    if (pruneResult && pruneResult.timedOut) {
      ...
      // Uses process.stderr.write to match the [gtd-tools] WARNING prefix style.
      process.stderr.write(
        '[gtd-tools] WARNING: worktree health check degraded' +
...
/**
 * Resolve the agents directory from the GTD install location.
 * gtd-tools.cjs lives at <configDir>/get-tasks-done/bin/gtd-tools.cjs,
 * so agents/ is at <configDir>/agents/.
 *
 * GTD_AGENTS_DIR env var overrides the default path. Used in tests and for
 * installs where the agents directory is not co-located with gtd-tools.cjs.
 *
 * @returns {string} Absolute path to the agents directory
 */
function getAgentsDir() {
  if (process.env.GTD_AGENTS_DIR) {
    return process.env.GTD_AGENTS_DIR;
  }
  // __dirname is get-tasks-done/bin/lib/ → go up 3 levels to configDir
 ...
 *
 * Recognises both standard format (gtd-planner.md) and Copilot format
 * (gtd-planner.agent.md). Copilot renames agent files during install (#1512).
 *
...
function _warnUnknownProfileOverrides(parsed, configLabel) {
  ...
  if (runtime && typeof runtime === 'string' && !KNOWN_RUNTIMES.has(runtime)) {
    ...
    if (!_warnedConfigKeys.has(key)) {
      ...
      try {
        process.stderr.write(
          `gtd: warning — config key "runtime" has unknown value "${runtime}". ` +
  ...
  for (const [overrideRuntime, tierMap] of Object.entries(overrides)) {
    if (!KNOWN_RUNTIMES.has(overrideRuntime)) {
      ...
      if (!_warnedConfigKeys.has(key)) {
        ...
        try {
          process.stderr.write(
            `gtd: warning — model_profile_overrides.${overrideRuntime}.* uses ` +
    ...
    for (const tierName of Object.keys(tierMap)) {
      if (!RUNTIME_OVERRIDE_TIERS.has(tierName)) {
        ...
        if (!_warnedConfigKeys.has(key)) {
          ...
          try {
            process.stderr.write(
              `gtd: warning — model_profile_overrides.${overrideRuntime}.${tierName} ` +
 ...
 * @param {string} agentType - Agent name (e.g. 'gtd-verifier').
