 ...
 * `.planning/codebase/STRUCTURE.md` map produced by `gtd-codebase-mapper`.
 *
...
function buildMessage(elements, affectedPaths, action, runtime) {
  ...
  if (action === 'auto-remap') {
    ...
    const { formatGtdSlash } = require('./runtime-slash.cjs');
    const mapCmd = formatGtdSlash('map-codebase', runtime || 'claude');
    lines.push(
