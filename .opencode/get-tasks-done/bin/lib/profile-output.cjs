...
function buildClaudeMdFallbacks(runtime) {
  return {
    project: `Project not yet initialized. Run ${formatGtdSlash('new-project', runtime)} to set up.`,
...
function buildClaudeMdWorkflowEnforcement(runtime) {
  return [
    ...
    `- \`${formatGtdSlash('quick', runtime)}\` for small fixes, doc updates, and ad-hoc tasks`,
    `- \`${formatGtdSlash('debug', runtime)}\` for investigation and bug fixing`,
    `- \`${formatGtdSlash('export-phase-issues', runtime)}\`, then \`${formatGtdSlash('work-task-issue', runtime)}\` or \`${formatGtdSlash('orchestrate-tasks', runtime)}\`, then \`${formatGtdSlash('work-task-issue', runtime)} --complete-phase <phase> --execute\` for planned phase work`,
    '',
...
function buildClaudeMdProfilePlaceholder(runtime) {
  return [
    ...
    '## Developer Profile',
    '',
    `> Profile not yet configured. Run \`${formatGtdSlash('profile-user', runtime)}\` to generate your developer profile.`,
...
function buildSection(sectionName, sourceFile, content) {
  return [
    `<!-- GTD:${sectionName}-start source:${sourceFile} -->`,
    content,
    `<!-- GTD:${sectionName}-end -->`,
  ].join('\n');
}
...
function generateSkillsSection(cwd) {
  ...
  for (const dir of SKILL_SEARCH_DIRS) {
    ...
    for (const entry of entries) {
      ...
      // Skip GTD's own installed skills — only surface project-specific skills
...
function cmdWriteProfile(cwd, options, raw) {
  ...
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'get-tasks-done', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock = `Stack preferences not available (questionnaire-only profile). Run \`${formatGtdSlash('profile-user', resolveRuntime(cwd))} --refresh\` with session data to populate.`;
  ...
  // commands/gtd subdirectory in favor of skills/<skill>/SKILL.md under
  // the runtime config dir. This writer was missed in the migration
  // (PR #1540 targeted GTD-shipped command files; dev-preferences is a
  // runtime-generated user artifact). Default now points at the skills/
  // location so /gtd:profile-user --refresh stops re-creating the legacy
  ...
  if (!outputPath) {
    ...
    try {
      ...
      effectiveRuntime = process.env.GTD_RUNTIME || config.runtime || 'claude';
    } catch {
      effectiveRuntime = process.env.GTD_RUNTIME || 'claude';
    }
    const skillDir = getGlobalSkillDir(effectiveRuntime, 'gtd-dev-preferences');
    if (!skillDir) {
  ...
  const result = {
    ...
    command_name: formatGtdSlash('dev-preferences', resolveRuntime(cwd)),
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by GTD from ${dataSource}. Run \`${formatGtdSlash('profile-user', resolveRuntime(cwd))} --refresh\` to update.`,
    '',
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  try {
    ...
    // GTD_RUNTIME env var takes precedence over config.runtime, mirroring detectRuntime().
    const effectiveRuntime = process.env.GTD_RUNTIME || config.runtime || null;
  ...
  if (existingContent === null) {
    ...
    if (!options.auto && fileContent.indexOf('<!-- GTD:profile-start') === -1) {
  ...
  if (profileStatus === 'placeholder_added') message += ` Run ${formatGtdSlash('profile-user', resolveRuntime(cwd))} to unlock Developer Profile.`;
