 ...
 *   2. User-level defaults from ~/.gtd/defaults.json (if present)
 *   3. userChoices — the settings the user explicitly selected during /gtd:new-project
 *
...
function buildNewProjectConfig(userChoices) {
  ...
  // Load user-level defaults from ~/.gtd/defaults.json if available
  const globalDefaultsPath = path.join(homedir, '.gtd', 'defaults.json');
  let userDefaults = {};
  try {
 ...
 * hardcoded defaults and optional ~/.gtd/defaults.json.
 *
 ...
 * users can run `gtd-tools migrate-config` to apply all four legacy-key
...
async function cmdMigrateConfig(cwd, raw) {
  ...
  const ws = process.env.GTD_WORKSTREAM || null;
