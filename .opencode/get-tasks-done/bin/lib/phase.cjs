...
// Documented in agents/gtd-planner.md (write_phase_prompt step). The wider
...
function describeNonCanonicalPlans(dirFiles, matchedFiles) {
  ...
  return (
    ...
    `See agents/gtd-planner.md write_phase_prompt step for the full contract.`
  );
}
...
function cmdPhaseAdd(cwd, description, raw, customId) {
  ...
  const { newPhaseId, dirName } = withPlanningLock(cwd, () => {
    ...
    const phaseEntry = `\n### Phase ${_newPhaseId}: ${description}\n\n**Goal:** [To be planned]\n**Requirements**: TBD${dependsOn}\n**Plans:** 0 plans\n\nPlans:\n- [ ] TBD (run ${formatGtdSlash('plan-phase', resolveRuntime(cwd))} ${_newPhaseId} to break down)\n`;
...
function cmdPhaseAddBatch(cwd, descriptions, raw) {
  ...
  const results = withPlanningLock(cwd, () => {
    ...
    for (const description of descriptions) {
      ...
      const phaseEntry = `\n### Phase ${newPhaseId}: ${description}\n\n**Goal:** [To be planned]\n**Requirements**: TBD${dependsOn}\n**Plans:** 0 plans\n\nPlans:\n- [ ] TBD (run ${formatGtdSlash('plan-phase', resolveRuntime(cwd))} ${newPhaseId} to break down)\n`;
      const lastSeparator = rawContent.lastIndexOf('\n---');
...
function cmdPhaseInsert(cwd, afterPhase, description, raw) {
  ...
  const { decimalPhase, dirName } = withPlanningLock(cwd, () => {
    ...
    // Build phase entry
    const phaseEntry = `\n### Phase ${_decimalPhase}: ${description} (INSERTED)\n\n**Goal:** [Urgent work - to be planned]\n**Requirements**: TBD\n**Depends on:** Phase ${afterPhase}\n**Plans:** 0 plans\n\nPlans:\n- [ ] TBD (run ${formatGtdSlash('plan-phase', resolveRuntime(cwd))} ${_decimalPhase} to break down)\n`;
...
function cmdPhaseComplete(cwd, phaseNum, raw) {
  ...
  if (fs.existsSync(roadmapPath)) {
    withPlanningLock(cwd, () => {
      ...
      // block (the standard /gtd:new-project layout). replaceInCurrentMilestone
