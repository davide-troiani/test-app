...
// runs without a TTY (typical /gtd:update path via Claude Code or any
// scripted update), prompt-user migration actions cannot be answered
// interactively. We resolve them by classification:
//   - Stale SDK build artifacts (get-tasks-done/sdk/{dist,src}/gtd-*):
//     default `remove`. Fresh install supplies replacements.
//   - User-facing skill anchors (skills/gtd-*/SKILL.md): default `keep`.
...
const RESOLUTION_ENV_VAR = 'GTD_INSTALLER_MIGRATION_RESOLVE';
...
// set — a shape regex like `^hooks/gtd-[^/]+\.(?:js|sh|cjs|mjs)$` also
...
//
// Stale SDK build artifacts live under get-tasks-done/sdk/{dist,src}/
...
function classifyPromptUserAction(action) {
  ...
  if (/^get-tasks-done\/sdk\/(dist|src)\//.test(relPath)) {
    return { category: 'stale-sdk-build-artifact', choice: 'remove' };
  }
  if (/^skills\/gtd-[^/]+\/SKILL\.md$/.test(relPath)) {
    return { category: 'user-facing-skill', choice: 'keep' };
  }
  // #3610 / #3628: bundled GTD hooks shipped under `hooks/`. The whitelist
  ...
  if (BUNDLED_GTD_HOOK_FILES.has(relPath)) {
    return { category: 'bundled-gtd-hook', choice: 'remove' };
  }
  return null;
}
...
// migration journal under gtd-migration-journal/<runId>-backups/).
