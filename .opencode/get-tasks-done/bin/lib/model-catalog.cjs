...
//
//   1. Co-located install path — get-tasks-done/bin/shared/model-catalog.json
...
//      ai-is-gonna/get-tasks-done clone (the original path introduced by #3230).
//
//   3. GTD_MODEL_CATALOG env override — allows test harnesses and custom
...
const _catalogCandidates = [
  ...
  process.env.GTD_MODEL_CATALOG ? path.resolve(process.env.GTD_MODEL_CATALOG) : null,
].filter(Boolean);
