...
function tryLoadSdk() {
  ...
  try {
    const sdkModule = require('@ai-is-gonna/gtd-sdk');
...
function dispatchViaSdk(registryCommand, registryArgs, legacyArgs, cwd, raw, error, rawFormatter) {
  ...
  const result = _executeForCjs({
    ...
    mode: raw ? 'raw' : 'json',
    projectDir: cwd,
    // workstream: not threaded here — GTDTransport forces subprocess for workstream
 ...
 * Keeps gtd-tools.cjs thin while preserving existing command semantics.
 *
 ...
 * - Any command when GTD_WORKSTREAM is active (GTDTransport forces subprocess
...
function routeStateCommand({ state, args, cwd, raw, parseNamedArgs, error }) {
  ...
  // Workstream guard: if GTD_WORKSTREAM is set, the sync bridge worker cannot
  // handle the request (GTDTransport.subprocessReason returns 'workstream_forced'
  // and subprocess is disabled in the worker). Fall back to CJS path for all
  // workstream-scoped state commands.
  const activeWorkstream = process.env.GTD_WORKSTREAM;
  ...
  routeCjsCommandFamily({
    ...
    unsupported: {
      'add-roadmap-evolution': 'state add-roadmap-evolution is SDK-only. Use: gtd-sdk query state.add-roadmap-evolution ...',
    },
    error,
