...
<task name="{fix_description}" type="auto">
  ...
  <read_first>{artifact.path}, {relevant SUMMARY/source files}</read_first>
  <boundaries>
    Allowed: write only {artifact.path}.
    Forbidden: {related files/dirs owned by other tasks}
    Out of scope: deferred items and unrelated verification gaps.
  </boundaries>
  <action>
  ...
  <acceptance_criteria>
    - {Mechanical proof the missing item is fixed}
  </acceptance_criteria>
