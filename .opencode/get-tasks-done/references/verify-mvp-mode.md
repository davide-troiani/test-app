...
> Loaded by `verify-work` workflow and `gtd-verifier` agent only when the phase under verification has `mode: mvp` in ROADMAP.md. Reframes UAT generation from technical checks to user-flow walk-throughs.
...
- The phase under verification has `**Mode:** mvp` in ROADMAP.md (parsed via `gtd-sdk query roadmap.get-phase --pick mode`).
- AND the phase has a user-story-formatted goal (set by `/gtd mvp-phase` per Phase 2): "As a [user role], I want to [capability], so that [outcome]."

If the phase has `mode: mvp` but the goal is NOT in user-story format, the verifier surfaces this as a discrepancy and asks the user to run `/gtd mvp-phase` to reformat the goal — same pattern as the planner agent under MVP_MODE (per `references/planner-mvp-mode.md`).
...
1. The first step opens the entry point ("Open the app", "Navigate to /register", "Run `gtd mvp-phase 1`").
...
Coverage is a goal-backward check: "did this phase deliver what its user story promised?" — sourced from the existing `gtd-verifier` agent's goal-backward methodology, narrowed to the user story.
...
The `gtd-verifier` agent produces `VERIFICATION.md`. Under MVP mode, the report adds a top-level "User Flow Coverage" section that maps each step of the user story to evidence in the codebase:
